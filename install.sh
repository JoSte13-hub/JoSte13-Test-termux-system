...
if [ -f "$HOME/user/user.py" ]; then
    USERNAME=$(python3 -c "import sys; sys.path.insert(0, '$HOME/user'); import user; print(user.username)")
else
    USERNAME="unknown"
fi
export PS1="\[\e[32m\]┌──(\[\e[34m\]$USERNAME@JoSte13\[\e[32m\])─[\[\e[37m\]\w\[\e[32m\]]\n\[\e[32m\]└─\[\e[34m\]$\[\e[0m\] "
EOF
fi
